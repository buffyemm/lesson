...
string itemLib[] = { "axe","sword","shield", "key" }; // создал массив с именами предметов, в который будут передаваться предметы 
...
void InitGame() {       // создаем функцию которая будет заносить все наши значения в массив room
    ...
    room[1].item.push_back((item_::sheld));
    room[1].enemy.push_back({ "GOBLIN", 200, 10, true, {item_::sword, item_::sheld }, 1});
...
void fight() {
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < room[i].enemy.size(); i++) {

            if (user.current_loc == room[i].enemy[i].current_loc) {

                cout << " You were attacked " << room[user.current_loc].enemy[i].name;
                room[user.current_loc].attack = true;
    ...
    while (user.life) { // цикл игры, пока пользователь живой
        ...
        if (chouse == "use") {
            ...
            case 1: {
                ...
                if (room[user.current_loc].attack) {
