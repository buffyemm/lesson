...
string itemLib[3] = { "axe","sword","hemlet" }; // создал массив с именами предметов, в который будут передаваться предметы 
...
// на прошлой ветке предметы отображались в цифрах, в этой будут имена предметов
int main()
{
